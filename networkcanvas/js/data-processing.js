...
export function runAnalysis(tableData) {
    if (tableData.length === 0) {
        showNotification("Data table is empty. Please add data or import a CSV.", 'error');
        return null;
    }
    ...
    tableData.forEach(row => {
        ...
        for (let k = 2; k < 8; k++) { // Add codes from nomination columns
...
export function applyFiltersToMasterData(masterData, activeFilters) {
    ...
    const hasActiveDemographicFilters = activeFilters && Object.values(activeFilters).some(val => val.size > 0);
...
export function deleteNode(nodeId) {
    ...
    appState.previousMasterSociogramData = JSON.parse(JSON.stringify(appState.masterSociogramData));
    appState.canUndo = true;

    // Remove node from master data
    const nodeToDelete = appState.masterSociogramData.nodes.find(node => node.id === nodeId);
    appState.masterSociogramData.nodes = appState.masterSocigramData.nodes.filter(node => node.id !== nodeId);

    // Remove links connected to the deleted node from master data
    appState.masterSociogramData.links = appState.masterSociogramData.links.filter(link => {
        const sourceId = typeof link.source === 'object' ? link.source.id : link.source;
        const targetId = typeof link.target === 'object' ? link.target.id : link.target;
    ...
    showNotification(`Node "${nodeToDelete?.label || nodeId}" deleted.`, 'info', 6000, {
        text: 'Undo',
...
export function undoLastDelete() {
    ...
    if (appState.canUndo && appState.previousMasterSociogramData) {
        state.set({
            masterSociogramData: JSON.parse(JSON.stringify(appState.previousMasterSociogramData)),
            previousMasterSociogramData: null, // Clear previous state after undo
            canUndo: false,
        });
        reprocessDataAndRedrawAll();
        showNotification('The last deletion has been successfully undone.', 'success');
    } else {
...
export function mergeDemographicsIntoNodes(nodes, demographicsMap) {
    ...
    nodes.forEach(node => {
        if (demographicsMap.has(node.id)) {
            node.demographics = { ...node.demographics, ...demographicsMap.get(node.id) };
        }
    });
}

export function calculateAMS(nodes, links, groupAttribute, statusAttribute, statusMap, normFactor, homophilyWeights, w_status_factor) {
    calculateAMSContribution(nodes, links, groupAttribute, statusAttribute, statusMap, normFactor, homophilyWeights, w_status_factor);
}

export function calculateAMSContribution(nodes, links, groupAttribute, statusAttribute, statusMap, normFactor, homophilyWeights, w_status_factor) {
    ...
    triangles.forEach(triad => {
        ...
        const v12 = linkMap.get(`${id1}-${id2}`) || linkMap.get(`${id2}-${id1}`) || 0;
        const v23 = linkMap.get(`${id2}-${id3}`) || linkMap.get(`${id3}-${id2}`) || 0;
        const v31 = linkMap.get(`${id3}-${id1}`) || linkMap.get(`${id1}-${id3}`) || 0;
        ...
        if (i_raw > 0) {
            ...
            // W_homophily
            const groups = new Set([node1.demographics[groupAttribute], node2.demographics[groupAttribute], node3.demographics[groupAttribute]]);
            ...
            const statusSpan = Math.max(status1, status2, status3) - Math.min(status1, status2, status3);
...
export function recalculateNodeMetricsForSubset(nodes, links) {
    ...
    for (const link of links) {
        ...
        if (!nominationsBySource.has(sourceId)) {
            nominationsBySource.set(sourceId, { positive: new Set(), negative: new Set() });
        }
        if (link.type === 'positive') {
    ...
    for (const node of nodes) {
        const myPositiveNoms = nominationsBySource.get(node.id)?.positive || new Set();
        myPositiveNoms.forEach(targetCode => {
            const targetNomsToMe = nominationsBySource.get(targetCode)?.positive || new Set();
