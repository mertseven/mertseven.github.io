...
export function renderFilterPanel() {
  ...
  if (!demographicKeys || demographicKeys.length === 0) {
    filterControlsContainer.innerHTML =
      '<p class="hint">No demographics available. Import data to create filters.</p>';
    return;
  }
    ...
    const values = [
      ...new Set(
        masterSociogramData.nodes
          .map((n) => n.demographics[key])
          .filter((v) => v),
      ),
    ];

    values.forEach((value) => {
      const count = masterSociogramData.nodes.filter(
        (n) => n.demographics[key] === value,
      ).length;
      const checkboxContainer = document.createElement("div");
      checkboxContainer.classList.add("checkbox-item");

      const isChecked = activeFilters[key]
        ? activeFilters[key].has(value)
        : true;
