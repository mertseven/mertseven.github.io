 ...
 */
export function createAppGraphToJsNetworkX(
  appNodes,
  appLinks,
  onlyPositive = true,
) {
...
export function findMaximalCliques(G) {
  ...
  try {
    ...
    if (
      cliquesGenerator &&
      typeof cliquesGenerator[Symbol.iterator] === "function"
    ) {
...
export function calculateBetweennessCentrality(G) {
  if (!jsnx) {
    console.error(
      "jsnetworkx module not available for calculateBetweennessCentrality.",
    );
    return new Map();
  }
  if (!G) return new Map();
  ...
  try {
    const betweenness = jsnx.betweennessCentrality(G, {
      normalized: true,
      weight: null,
    });
    return betweenness;
...
export function calculateClosenessCentrality(G) {
  if (!jsnx) {
    console.error(
      "jsnetworkx module not available for calculateClosenessCentrality.",
    );
    return new Map();
  }
  if (!G) return new Map();
  ...
  try {
    ...
    for (const component of components) {
      if (component.size > 1) {
        ...
        const componentCloseness = jsnx.closenessCentrality(subGraph, {
          wf_improved: true,
        });
...
export function calculateEigenvectorCentrality(G) {
  if (!jsnx) {
    console.error(
      "jsnetworkx module not available for calculateEigenvectorCentrality.",
    );
    return new Map();
  }
  if (!G) return new Map();
  ...
  try {
    const eigenvector = jsnx.eigenvectorCentrality(G, {
      maxIter: 1000,
      tol: 1e-6,
    });
    return eigenvector;
