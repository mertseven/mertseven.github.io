...
const demographicsTableHead = document.querySelector(
  "#demographics-table thead",
);
const demographicsTableBody = document.querySelector(
...
const groupAnalysisResultsContainer = document.getElementById(
  "group-analysis-results-container",
); // NEW
...
export function renderGroupAnalysisResults(results) {
  ...
  const normalityGroupSelect = document.getElementById(
    "normality-group-select",
  );
  const currentNormalityPlotContainer = document.getElementById(
    "current-normality-plot-container",
  );
  const normalityPlotTextSummary = document.getElementById(
    "normality-plot-text-summary",
  );
  ...
  function updateNormalityPlotAndText() {
    ...
    const selectedNormalityResult = normalityResults.find(
      (res) => res.groupName === selectedGroupName,
    );
...
export function populateCliqueSelector() {
  ...
  const filteredCliques = state.allFoundCliques.filter(
    (clique) => clique.length >= minSize,
  );
  filteredCliques.sort(
    (a, b) =>
      b.length - a.length || JSON.stringify(a).localeCompare(JSON.stringify(b)),
  );
...
export function updateMetricsTable(nodes) {
  ...
  nodes.forEach((node) => {
    const tr = document.createElement("tr");
    const socialPreferenceScore =
      node.preferencesReceived - node.nonPreferencesReceived;
    tr.innerHTML = `
      <td>${node.id}</td>
