...
// DOM Elements
const viewToolbarButtons = document.querySelectorAll(
  "#view-toolbar .toolbar-button[data-tab]",
);
...
const demographicsCsvFileInput = document.getElementById(
  "demographics-csv-file-input",
);
...
const nodeLabelSizeValueDisplay = document.getElementById(
  "node-label-size-value",
);
...
const chargeStrengthValueDisplay = document.getElementById(
  "charge-strength-value",
);
const collideRadiusFactorInput = document.getElementById(
  "collide-radius-factor",
);
const collideRadiusFactorValueDisplay = document.getElementById(
  "collide-radius-factor-value",
);
...
const addDemographicColumnBtn = document.getElementById(
  "add-demographic-column-btn",
);
...
function setupEventListeners() {
    ...
    button.addEventListener("click", () => {
      ...
      if (button.dataset.tab === "graph") {
        requestAnimationFrame(() => {
          ...
          if (
            appState.masterSociogramData.nodes.length > 0 &&
            appState.currentDisplayNodes.length === 0 &&
            appState.currentDisplayLinks.length === 0
          ) {
            applyFiltersAndRedraw();
  ...
  clearTableBtn.addEventListener("click", () => {
    ...
    if (appState.graphInitialized) {
      ...
      if (appState.simulation)
        appState.simulation.nodes([]).force("link", null).alpha(1).stop();
    }
    populateCliqueSelector();
    ...
    updateLegendAndExplanation(nodeColorModeSelect.value);
    if (
      appState.zoomBehavior &&
      appState.svg.node() &&
      appState.graphInitialized
    ) {
    ...
    showNotification("Table cleared.", "info", 6000, {
      ...
      onClick: () => {
        ...
        const dataForTable = previousState.masterSociogramData.nodes.map(
          (node) => {
            const links = previousState.masterSociogramData.links.filter(
  ...
  analyzeBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
      if (analysisResult) {
        ...
        const graphTabButton = document.querySelector(
          '.toolbar-button[data-tab="graph"]',
        );
        if (graphTabButton) {
          if (!document.getElementById("graph").classList.contains("active")) {
            ...
            requestAnimationFrame(() => {
              ...
              if (!appState.graphInitialized) {
                if (!setupD3Graph()) {
                  console.error(
                    "Graph setup failed during analysis while tab was active.",
                  );
                  return;
                }
              }
              const containerRect =
                appState.d3Container.getBoundingClientRect();
              if (containerRect.width > 0 && containerRect.height > 0) {
                setState({
                ...
                if (appState.simulation) {
                  appState.simulation.force(
                    "center",
                    d3.forceCenter(appState.width / 2, appState.height / 2),
                  );
                }
              }
              applyFiltersAndRedraw();
  ...
  addDemographicColumnBtn.addEventListener("click", () => {
    const newKey = prompt(
      "Enter a name for the new demographic (e.g., Age, Department):",
    );
    if (newKey && newKey.trim() !== "") {
      ...
      if (appState.demographicKeys.includes(trimmedKey)) {
        showNotification(
          `The demographic "${trimmedKey}" already exists.`,
          "error",
        );
        return;
      }
      if (appState.masterSociogramData.nodes.length === 0) {
        showNotification(
          "Please click 'GET ANALYSIS' at least once before adding demographics.",
          "info",
        );
        return;
      }
  ...
  demographicsTable.addEventListener("click", (event) => {
    if (event.target.classList.contains("delete-column-btn")) {
      ...
      if (
        confirm(
          `Are you sure you want to delete the entire "${keyToDelete}" demographic column?`,
        )
      ) {
        const appState = getState();
        appState.demographicKeys = appState.demographicKeys.filter(
          (k) => k !== keyToDelete,
        );
  ...
  demographicsTable.addEventListener(
    "blur",
    (event) => {
      ...
      if (target.tagName === "TD" && target.contentEditable === "true") {
        ...
        const nodeToUpdate = appState.masterSociogramData.nodes.find(
          (n) => n.id === code,
        );
        if (nodeToUpdate) {
      ...
      if (target.tagName === "SPAN" && target.contentEditable === "true") {
        ...
        if (!newKey || appState.demographicKeys.includes(newKey)) {
          showNotification(
            `Invalid name. "${newKey}" is either empty or already exists.`,
            "error",
          );
          target.textContent = oldKey;
          return;
        }
        const keyIndex = appState.demographicKeys.findIndex(
          (k) => k === oldKey,
        );
        if (keyIndex > -1) {
  ...
  if (demographicsTableThead) {
    demographicsTableThead.addEventListener("click", (event) => {
      ...
      if (demogCurrentSort.column === column) {
        demogCurrentSort.direction =
          demogCurrentSort.direction === "asc" ? "desc" : "asc";
      } else {
      ...
      header.classList.add(
        demogCurrentSort.direction === "asc" ? "sorted-asc" : "sorted-desc",
      );
    });
  }
  ...
  if (metricsTableThead) {
    metricsTableThead.addEventListener("click", (event) => {
      ...
      if (metricsCurrentSort.column === column) {
        metricsCurrentSort.direction =
          metricsCurrentSort.direction === "asc" ? "desc" : "asc";
      } else {
      ...
      nodesToSort.sort((a, b) => {
        ...
        if (typeof valA === "string") {
          ...
          return metricsCurrentSort.direction === "asc"
            ? comparison
            : -comparison;
        } else {
      ...
      header.classList.add(
        metricsCurrentSort.direction === "asc" ? "sorted-asc" : "sorted-desc",
      );
    });
  }
  ...
  [linkDistanceInput, chargeStrengthInput, collideRadiusFactorInput].forEach(
    (input) => {
      input.addEventListener("input", () => {
        if (input.id === "link-distance")
          linkDistanceValueDisplay.textContent = input.value;
        if (input.id === "charge-strength")
          chargeStrengthValueDisplay.textContent = input.value;
        if (input.id === "collide-radius-factor")
          collideRadiusFactorValueDisplay.textContent = input.value;
        const appState = getState();
        if (appState.simulation && appState.currentDisplayNodes.length > 0) {
          if (input.id === "link-distance")
            appState.simulation.force("link").distance(+input.value);
          if (input.id === "charge-strength")
            appState.simulation.force("charge").strength(+input.value);
          if (input.id === "collide-radius-factor")
            appState.simulation
              .force("collide")
              .radius((d) => d.radius * +input.value + 3);
          appState.simulation.alpha(0.3).restart();
  ...
  [
    showPositiveLinksCheckbox,
    showNegativeLinksCheckbox,
    minPrefsFilterInput,
  ].forEach((input) => {
  ...
  downloadGraphBtn.addEventListener("click", async () => {
    ...
    const options = {
      backgroundColor:
        getComputedStyle(appState.d3Container).backgroundColor ||
        "var(--bg-primary)",
      scale: 2,
    };
  ...
  minCliqueSizeInput.addEventListener("input", () => {
    const appState = getState();
    if (
      appState.allFoundCliques.length > 0 ||
      appState.masterSociogramData.nodes.length > 0
    ) {
      populateCliqueSelector();
      clearHighlights();
    ...
    if (selectedOption && selectedOption.value !== "") {
      try {
        ...
        if (visibleCliqueMembers.length >= 2) {
          ...
          clearHighlights();
          showNotification(
            "Selected clique members are not sufficiently visible with current graph filters.",
            "info",
          );
        }
      } catch (e) {
  ...
  // Export Listeners
  document
    .getElementById("export-metrics-csv-btn")
...
function saveState() {
  ...
  delete stateToSave.graphInitialized;
  ...
  reader.onload = (e) => {
    try {
      ...
      const dataForTable = loadedState.masterSociogramData.nodes.map((node) => {
        const links = loadedState.masterSociogramData.links.filter(
          (l) => (l.source.id || l.source) === node.id,
        );
        const mp = links
          .filter((l) => l.type === "positive")
...
function applyActiveFilters() {
  const activeFilters = {};
  document
    .querySelectorAll('#filter-controls-container input[type="checkbox"]')
    .forEach((checkbox) => {
