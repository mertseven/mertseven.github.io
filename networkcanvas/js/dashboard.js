...
export function updateDashboard() {
  ...
  if (!nodes || nodes.length === 0) {
    grid.innerHTML =
      "<p class=\"hint\">No data to display. Please run an analysis on the 'Data' tab first.</p>";
    return;
  }
  ...
  const sortedByBetweenness = [...nodes].sort(
    (a, b) => (b.betweenness || 0) - (a.betweenness || 0),
  );
  const sortedByEigenvector = [...nodes].sort(
    (a, b) => (b.eigenvector || 0) - (a.eigenvector || 0),
  );
