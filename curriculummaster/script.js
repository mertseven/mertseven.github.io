document.addEventListener('DOMContentLoaded', () => {
    ...
    // New D3 Controls for Tag-to-Tag links
    ...
    if(d3ControlParametersCollapsible) {
     ...
     if(d3TagLinkParametersCollapsible) { // Setup for new collapsible section
        ...
        if (d3TagLinkToggleButton) {
            d3TagLinkParametersCollapsible.querySelector('.d3-control-header').addEventListener('click', () => {
                ...
                d3TagLinkToggleButton.textContent = d3TagLinkParametersCollapsible.classList.contains('expanded') ? '▲' : '▼';
                 // Toggle content display directly for immediate effect
             ...
             // Initialize based on current state (e.g., if you want it expanded by default)
    ...
    function generateD3GraphData() {
        ...
        // Conditionally add Tag-to-Tag links
    ...
    function calculateDegreeCentrality(nodes, links) {
        ...
        links.forEach(link => {
            // Ensure link.source and link.target are node objects if D3 has processed them,
            // otherwise use them as IDs.
    ...
    function calculateBetweennessCentrality(nodes, links) {
        ...
        links.forEach(link => {
            const u = link.source.id || link.source; // Handle D3 object or ID string
            const v = link.target.id || link.target; // Handle D3 object or ID string
        ...
        for (const sNode of nodes) {
            ...
            while (S.length > 0) {
                ...
                if (w !== s) {
                    ...
                    if (wNode) wNode.betweenness += delta.get(w); // Add to target, not source
                }
            }
        }
         // For undirected graphs, Brandes' algorithm counts each path twice.
        // For nodes with degree 1, their betweenness should strictly be 0.
        nodes.forEach(node => {
    ...
    function initializeAndDisplayD3TagNetwork() {
        ...
        generateD3GraphData(); // This now also handles tag-to-tag links based on toggle
        calculateDegreeCentrality(currentD3GraphData.nodes, currentD3GraphData.links);
        networkMetricsCalculated = false; 
        currentD3GraphData.nodes.forEach(n => n.betweenness = undefined); // Reset for display
        ...
        d3LinkElements = d3Svg.append("g").attr("class", "d3-links").selectAll("line")
            ...
            .attr("stroke-width", d => d.type === 'tag-tag' ? 1.2 : Math.max(1, Math.sqrt(d.value || 1) * 1.5)) // Adjusted for visual distinction
            .attr("stroke", d => d.type === 'tag-tag' ? '#e74c3c' : '#555'); // Reddish for tag-tag
    ...
    function d3NodeClicked(event, d_node_data) {
        ...
        currentD3GraphData.links.forEach(link => {
            ...
            if (connectedNodeDatum && link.type !== 'tag-tag') { // Only show direct course-tag connections in this list for now
    ...
    function d3NodeMouseOver(event, d_node_data) {
        ...
        d3LinkElements.filter(l => (l.source.id === d_node_data.id || l.target.id === d_node_data.id) || (l.source === d_node_data.id || l.target === d_node_data.id) )
            ...
            .attr('stroke', d => d.type === 'tag-tag' ? '#e74c3c' : '#333') // Highlight with respective color
    ...
    function d3NodeMouseOut(event, d_node_data) {
        ...
        d3LinkElements.classed('dimmed', false).style('stroke-opacity', 0.6)
            .attr('stroke', d => d.type === 'tag-tag' ? '#e74c3c' : '#555')
            .attr('stroke-width', d => d.type === 'tag-tag' ? 1.2 : Math.max(1, Math.sqrt(d.value || 1) * 1.5));
        applyD3FiltersAndSearch(); 
    }

    function d3Drag(simulation) {
        function dragstarted(event, d) { if (!event.active) simulation.alphaTarget(0.3).restart(); d.fx = d.x; d.fy = d.y; if(d3GraphContainer) d3GraphContainer.classList.add('grabbing'); }
        function dragged(event, d) { d.fx = event.x; d.fy = event.y; }
        function dragended(event, d) { if (!event.active) simulation.alphaTarget(0); d.fx = null; d.fy = null; if(d3GraphContainer) d3GraphContainer.classList.remove('grabbing');}
            ...
            graphData.links.forEach(link => {
                ...
                if (neighborId) {
                    ...
                    if (newDepth <= maxDepth) {
                        if (!visitedDepths.has(neighborId)) { 
    ...
    function drawNeighborVisualization(centralNodeData, neighborsByDepth) {
        ...
        for (let depth = 1; depth <= maxDepthVisible; depth++) {
            const nodesAtThisDepth = neighborsByDepth[depth] || [];
            if (nodesAtThisDepth.length === 0) continue;
            const ringRadius = depth * baseRadiusStep + centralNodeRadius * 0.5; 
            const angleStep = (2 * Math.PI) / nodesAtThisDepth.length;
            nodesAtThisDepth.forEach((node, i) => {
                ...
                if (nodesAtThisDepth.length < 20 || neighborNodeRadius > 5) { 
    ...
    // Event Listeners for D3 Graph & Controls
    ...
    if (d3ToggleTagLinksCheckbox) {
        d3ToggleTagLinksCheckbox.addEventListener('change', function() {
            ...
            networkMetricsCalculated = false; // Metrics are now stale
            if (networkMetricsModal.style.display === 'block') { // Close metrics table if open
                closeNetworkMetricsModalButton.click();
            }
            initializeAndDisplayD3TagNetwork(); // Regenerate graph
        });
    }
    ...
    if (d3ResetViewButton) d3ResetViewButton.addEventListener('click', () => {
        ...
        if (d3ToggleTagLinksCheckbox) { // Reset tag link controls
        ...
        initializeAndDisplayD3TagNetwork(); //This will call updateD3SimulationParameters implicitly after graph regen
    ...
    if (d3ShowNeighborGraphButton) d3ShowNeighborGraphButton.addEventListener('click', () => {
        if (!currentCentralNodeForNeighborVis || !currentD3GraphData || !currentD3GraphData.nodes || !currentD3GraphData.links) { alert("Central node or graph data unavailable."); return; }
        const maxDepth = parseInt(neighborVisDepthSelect.value);
        const neighbors = getNDegreeNeighbors(currentCentralNodeForNeighborVis.id, maxDepth, currentD3GraphData);
        if (neighborVisTitle) neighborVisTitle.textContent = `Local Network for: ${currentCentralNodeForNeighborVis.displayName || currentCentralNodeForNeighborVis.name}`;
        if (neighborVisModal) neighborVisModal.style.display = 'block';
        if (d3NodePopup) d3NodePopup.classList.remove('visible');
        requestAnimationFrame(() => drawNeighborVisualization(currentCentralNodeForNeighborVis, neighbors));
    });
    if (neighborVisDepthSelect) neighborVisDepthSelect.addEventListener('change', () => {
        if (neighborVisModal && neighborVisModal.style.display === 'block' && currentCentralNodeForNeighborVis) {
            const maxDepth = parseInt(neighborVisDepthSelect.value);
            const neighbors = getNDegreeNeighbors(currentCentralNodeForNeighborVis.id, maxDepth, currentD3GraphData);
            drawNeighborVisualization(currentCentralNodeForNeighborVis, neighbors);
        }
    });
    ...
    if (d3MinCooccurrenceInput && d3ToggleTagLinksCheckbox) d3MinCooccurrenceInput.disabled = !d3ToggleTagLinksCheckbox.checked; // Initial state of min cooc input
